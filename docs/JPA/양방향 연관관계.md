...
<img src="/static/img/6.png" width="500px" height="300px" title="table2" alt="2개의 table 생성됨"></img><br/>
...
- **누구를 주인으로?**
    ...
    - 외례키 가 있는 곳이 항상 N(다) 이다. (Member : Team = N : 1),
    그러면 @ManyToOne (N:1 상황에서) 이 된다. 이쪽이 항상 연관관계의 주인.

    - 엔티티와 테이블이 매핑되어 있는 테이블에서 FK가 관리가 된다.

    -  ex) Member 엔티티를 바꿧더니 Member update 쿼리가 나가는 직관성을 갖을 수 있다.
    ...
    ```
    <img src="/static/img/7.png" width="400px" height="200px" title="table2" alt="연관관계 주인"></img><br/>
 ...
 //     team.getMembers().add(member);
    ...
    ```
    <img src="/static/img/8.png" width="400px" height="200px" title="table2" alt="연관관계 주인"></img><br/>
...
```
<img src="/static/img/9.png" width="150px" height="50px" title="result" alt="올바르게 들어감"></img><br/>
...
* 연관관계 편의 메소드 이용
  ...
  - 연관관계 편의 메소드는 한쪽에만 넣어준다. => 양방향 매핑 시 무한루프 걸릴 수 있다.
    ```
    //Member 클래스에 생성
