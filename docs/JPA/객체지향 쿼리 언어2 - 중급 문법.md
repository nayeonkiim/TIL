...
<img src="https://github.com/nayeonkiim/TIL/blob/master/docs/JPA/img/1.JPG" width="700px" height="400px" title="table1" alt="값타입"></img><br/>
...
```
- team 을 select 하면 team을 기준으로 member의 수를 조회하므로 팀A에 대한 Team 객체가 2개 생긴다. 그리고 각 Team 객체 대해 member의 size를 출력하면 2가 나온다.<br/>
<img src="https://github.com/nayeonkiim/TIL/blob/master/docs/JPA/img/2.JPG" width="450px" height="100px" title="table1" alt="join테이블"></img><br/>
...
- **JPQL의 DISITNCT** 2가지 기능을 제공한다.
    - SQL에 DISTINCT를 추가
    - 결과가 애플리케이션에 올라오면 **동일한 엔티티**를 제거. 여기서 같은 식별자를 가진 Team 엔티티를 제거
...
- 페치 조인 대상에는 별칭을 줄 수 없다.
    ...
    - 특정 대상을 걸러서 가져올 수 없다. 위의 코드는 불가능.
    - `String query = "select t From Team t join fetch t.members m join fetch m.team";`
        - 이렇게 fetch를 여러 단계로 사용해야 하는 경우만 별칭 사용한다.
    - 대상과 연관된 row 일부를 가져오고 싶다면 페치 조인 대신 따로 쿼리를 통해 조회하는 것이 맞다.
