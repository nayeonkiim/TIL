...
SELECT * FROM sample;
```
- mysql의 경우 데이터를 추가하거나 갱신할 경우 동일한 테이블을 서브쿼리에서 사용할 수 없도록 되어 있다.<br/>
    <img src="https://github.com/nayeonkiim/TIL/blob/master/docs/Database/img/7.JPG?raw=true" width="700px" height="160px" title="db" alt="subquery"></img><br/>
    - `DELETE FROM sample WHERE a = (SELECT a FROM (SELECT MIN(a) AS a FROM sample2) AS x);`<br/>
    <img src="https://github.com/nayeonkiim/TIL/blob/master/docs/Database/img/7.JPG?raw=true" width="700px" height="160px" title="db" alt="subquery"></img><br/>
