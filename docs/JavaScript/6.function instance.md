...
2. Book.prototype.getPoint = function(){}
    ...
    - Book.prototype이 **오브젝트**이므로 **프로퍼티를 연결**할 수 있다.
...
- 프로퍼티 공유 시점
    - 사용하는 시점에 prototype의 프로퍼티를 공유한다. 즉 메소드를 호출하는 시점이다.
    - prototype의 프로퍼티로 인스턴스를 생성하지만 인스턴스의 프로퍼티는 원본 property의 프로퍼티를 참조한다. 복사하지 않는다.
